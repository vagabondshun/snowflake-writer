...
## Version 1.6 - 2025-12-18

### 🎛️ RAG开关系统

这是一个专注于**用户体验优化**的版本，为RAG风格系统添加了持久化的开关控制功能。

#### 1. 新增功能 ✨

**持久化开关系统**:
- ✅ `enable_style_rag()` - 启用RAG并保存状态到metadata.json
- ✅ `disable_style_rag()` - 禁用RAG并释放内存资源
- ✅ `toggle_style_rag()` - 一键切换RAG启用/禁用状态
- ✅ 项目加载时自动恢复RAG状态
...
# 下次加载项目时，RAG自动启用
load_project("我的小说")  # RAG状态自动恢复

# 临时禁用
disable_style_rag()  # 释放资源，状态持久化

# 快速切换
toggle_style_rag()  # 自动检测当前状态并切换
```

#### 2. 改进 🔧

**story_engine.py** (+83行):
- 新增 `disable_style_rag()` 方法
- 新增 `toggle_style_rag()` 方法
- 更新 `enable_style_rag()` - 添加状态持久化
- 更新 `load_project()` - 自动恢复RAG状态
- metadata.json 新增 `style_rag_enabled` 字段

**文档更新**:
- `RAG_USAGE.md` - 添加开关使用示例和说明
- API文档新增3个函数说明
- 使用示例更新
...
```python
init_project("我的小说")
enable_style_rag()  # 启用并持久化
add_style_reference_from_file("参考小说.txt")
```

**场景2: 临时关闭节省资源**
```python
disable_style_rag()  # 禁用并释放内存
# ... 进行其他操作 ...
enable_style_rag()   # 重新启用
```

**场景3: 快速切换**
```python
toggle_style_rag()  # 切换状态
```
